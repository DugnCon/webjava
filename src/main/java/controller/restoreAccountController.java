...
public class restoreAccountController extends baseSceneController {
	@FXML
	...
	private void handleSearchID() {
		...
		if(!id.isEmpty()) {
			ArrayList<lockaccount> arr = lockAccount.setNew().selectByCondition1(id);
			...
			explain.setText(arr.get(0).getExplain());
	...
	private void handleRestore() {
		...
		int res = lockAccount.setNew().delete(lock);
		if(res > 0) {
			clearField();
